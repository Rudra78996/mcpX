import { z } from "zod";
import { makeExtensionRequest, sanitizeSelector, createErrorResponse, } from "../utils/helpers.js";
...
export async function clickElement({ selector, tabId, timeoutMs, }) {
...
export async function hoverElement({ selector, tabId, timeoutMs, }) {
    if (!selector || selector.trim().length === 0) {
        return createErrorResponse("Selector parameter cannot be empty");
    }
    const sanitizedSelector = sanitizeSelector(selector);
...
export const clickToolDef = {
    name: "tab-click",
    description: "Click an element in the current tab by CSS selector. Optionally pass tabId.",
    schema: clickToolSchema,
    handler: clickElement,
};
export const hoverToolDef = {
    name: "hover",
    description: "Hover over an element in the current tab by CSS selector. Optionally pass tabId.",
