...
export async function scrollPage({ direction, distance, tabId, selector, timeoutMs, }) {
    const validDirections = ["up", "down", "left", "right", "top", "bottom"];
    if (!validDirections.includes(direction)) {
        return createErrorResponse(`Invalid direction. Must be one of: ${validDirections.join(", ")}`);
    }
...
export const scrollToolDef = {
    name: "tab-scroll",
    description: "Scroll the page or a specific element. Supports directions: up, down, left, right, top (scroll to top), bottom (scroll to bottom). Optionally specify a CSS selector to scroll within a specific element, distance in pixels, and tabId.",
