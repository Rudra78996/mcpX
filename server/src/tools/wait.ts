...
export const waitToolSchema = {
  ...
  value: z
    .any()
    .describe(
      "For type 'time': number (milliseconds), for 'element'/'navigation': string (selector/URL pattern)"
    ),
...
export async function wait({
  ...
  if (type === "time") {
    if (typeof value !== "number") {
      const errorMsg = `For time-based waits, value must be a number (milliseconds). Got ${typeof value}: ${JSON.stringify(
        value
      )}`;
  ...
  if (type === "element" || type === "navigation") {
    if (typeof value !== "string") {
      const errorMsg = `For ${type} waits, value must be a string. Got ${typeof value}: ${JSON.stringify(
        value
      )}`;
      console.error("Wait validation error:", errorMsg);
      return createErrorResponse(errorMsg);
...
export const waitToolDef = {
  name: "wait",
  description:
    "Wait for a specified condition. Types: 'time' (milliseconds), 'element' (CSS selector), 'navigation' (URL pattern)",
